...
| body 边界、JSON 或字段非法 | 400 | `{ "ok": false, "error": "bad_request" }` |
| 正在接收另一个 `/api/tx/send` body | 409 | `{ "ok": false, "error": "busy" }` |
| pending command 队列已满 | 503 | `{ "ok": false, "error": "queue_full" }` |

`analyzer_web` 的 ESPAsyncWebServer body callback 只做 body 边界检查、JSON 解析、字段校验和 pending command 入队；它不得直接调用 `TxService::sendSingle()` 或触碰底层 driver。真正的发送在 `analyzerWebLoop()` 现有 pending command 串行路径中执行，最终 `TxService` 结果仅通过 Serial log 记录，不新增 UI 状态。TX body 可以继续沿用全局 buffer，但必须用 busy gate 拒绝并发 body，避免分块 POST 互相覆盖。
...
5. 合法请求的 HTTP 200 仅表示已入队 pending，实际发送随后在 `analyzerWebLoop()` 中完成；前端按当前 P5b 范围显示“已提交/待处理”即可。
...
3. 目标通道离线时，请求被拒绝。
4. 非法 body、JSON、ID、DLC、byte 输入被前端或后端拒绝。
5. 合法 POST 立即返回 `{ "ok": true, "pending": true }`，实际发送在 `analyzerWebLoop()` pending command 路径中执行。
6. 并发 `/api/tx/send` body 被 busy gate 拒绝，不会覆盖全局 TX body buffer。
7. 连续快速点击发送时，第二次在 pending 执行阶段命中 10ms 限速并写入 Serial log。
8. Master ON、目标通道 TX ON、通道在线时，单帧发送成功。
9. 顶部 TX 横幅与 `/api/status` 保持一致。
