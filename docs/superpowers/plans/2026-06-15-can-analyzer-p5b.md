...
**Architecture:** Add `TxService` as the only backend path allowed to call `CanDriver::send()`. Wire `TxService` into `can_analyzer.cpp` and `analyzer_web`, expose `POST /api/tx/send`, and add a small frontend form that uses the existing TX banner/status state and confirms before sending. The Web body callback validates/parses and enqueues a pending command only; actual `TxService::sendSingle()` execution happens later in `analyzerWebLoop()` on the existing pending command serial path.
...
- Modify: `src/analyzer/analyzer_web.cpp` — hold `TxService*`, parse `POST /api/tx/send`, busy-gate the shared TX body buffer, enqueue TX pending commands, and execute `TxService::sendSingle()` from `analyzerWebLoop()`.
...
- [ ] **Step 3: Add JSON parser helpers and pending storage in analyzer_web.cpp**

Inside the anonymous namespace in `src/analyzer/analyzer_web.cpp`, add TX body storage guarded by request-owner busy state/critical section, and extend `PendingCmd` with a `TxSend` type carrying channel/id/dlc/data. The global body buffer may stay shared, but concurrent `/api/tx/send` body uploads must be rejected instead of interleaved.
...
In `analyzerWebBegin()`, near the existing `/api/can-tx*` routes, add the `/api/tx/send` route with these semantics:

- `index == 0` tries to acquire the TX body busy gate; if another TX body is active, return `409 {"ok":false,"error":"busy"}`.
- Each chunk only validates bounds and copies into `g_txBody`; malformed body releases the busy gate and returns 400.
- On the final chunk, parse JSON and fields into a `PendingCmdType::TxSend` payload.
- Enqueue with the existing `enqueuePendingCommand()` queue; if full, release the busy gate and return `503 {"ok":false,"error":"queue_full"}`.
- On successful enqueue, release the busy gate and return `200 {"ok":true,"pending":true}`.
- Do **not** call `g_txService->sendSingle()` from the body callback.

Then extend `processPendingCommand()` so `PendingCmdType::TxSend` calls `g_txService->sendSingle(cmd.channel, cmd.id, cmd.dlc, cmd.data, millis())` and logs the final result to Serial. This keeps CAN driver access on the existing `analyzerWebLoop()` pending command serial path.
...
1. Default TX OFF: a valid POST is accepted as pending, then pending execution logs `tx_disabled` and no frame is sent.
2. Master ON but target channel TX OFF: pending execution logs `tx_disabled`.
3. Target channel offline: pending execution logs `tx_disabled`.
4. Bad body, JSON, ID, DLC, or byte value is rejected before enqueue or by POST.
5. Concurrent `/api/tx/send` body uploads return `busy` instead of sharing `g_txBody`.
6. Two rapid successful pending sends hit `rate_limited` on the second pending execution.
7. Master ON + target channel TX ON + channel online sends one frame from the pending command path.
8. Top TX banner remains consistent with `/api/status`.
